...
class Model:
    def __init__(self, device, dtype, **kwargs):
        ...
        self.pipe_dict = {
            ...
            ModelType.TTVO: TextToVideoPipeline,
        ...
        self.pix2pix_attn_proc = utils.CrossFrameAttnProcessor(
            unet_chunk_size=3)
        self.TTVO_attn_proc = utils.CrossFrameAttnProcessor(
            unet_chunk_size=2)
    ...
    def inference_chunk(self, frame_ids, **kwargs):
        ...
        if self.model_type == ModelType.TTVO:
    ...
    def process_TTVO(self,
                           ...
                           path=None):
        print("Module TTVO")
        if self.model_type != ModelType.TTVO or model_name != self.model_name:
            print("Model update")
            unet = UNet2DConditionModel.from_pretrained(
                model_name, subfolder="unet")
            self.set_model(ModelType.TTVO,
                           model_id=model_name, unet=unet)
            self.pipe.scheduler = DDIMScheduler.from_config(
                self.pipe.scheduler.config)
            if use_cf_attn:
                self.pipe.unet.set_attn_processor(
                    processor=self.TTVO_attn_proc)
        self.generator.manual_seed(seed)
